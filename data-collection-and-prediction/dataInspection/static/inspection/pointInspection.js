...
class State {
    ...
    storeInDb(onRenderRequested) {
        ...
        var foamPrintAnalysis = this.pois[0].data.foamPrintAnalysis;

        var me = this;

        // get points in db
        $.ajax({
            url: Urls.foamprintpointList() + "?foot=" + foot + "&foamPrintAnalysis=" + foamPrintAnalysis,  // Replace <id> with the actual ID of the record you want to update
            ...
            success: function (response) {
                ...
                pointsInDb.forEach(pointInDb => {
                    ...
                    if (!foundPoint) {
                        ...
                        $.ajax({
                            url: Urls.foamprintpoint_detail(pointInDb.id),  // Replace <id> with the actual ID of the record you want to update
                ...
                me.pois.forEach((poi) => {
                    ...
                    if (poi.id == -1) { // this point is not in the db yet, save it
                        $.ajax({
                            url: Urls.foamprintpointList(),  // Replace <id> with the actual ID of the record you want to update
                        ...
                        $.ajax({
                            url: Urls.foamprintpoint_detail(poi.id),  // Replace <id> with the actual ID of the record you want to update
...
function renderPointEditor(container_id, image_url, points, foot, participant_id, allowEdit = true, initial_zoom = 0.5, window_width=700, window_height=1400, onMeassure = ()=>{}) {
    // Create HTML Stuff
    ...
    var foamPrintAnalysis = points[0].foamPrintAnalysis;
    ...
    if(allowEdit){
        ...
        newPointButton.on("click", function () {
            if (pointObjects.length > 0) {
                var data = {
                    "foamPrintAnalysis": pointObjects[0].greenAiInstance.foamPrintAnalysis,
