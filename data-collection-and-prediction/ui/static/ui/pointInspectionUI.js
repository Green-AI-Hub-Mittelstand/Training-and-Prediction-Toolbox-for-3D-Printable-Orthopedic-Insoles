...
class State {
    ...
    storeInDb(onRenderRequested, foot) {
        console.log("storing in db");
        console.log(Urls.predictedpointleftList());
        // we need to find 
        ...
        // for each points, update it in db
        this.pois.forEach((point, index)=>{
            console.log(point);

            if (point['data']['x'] == point['x'] && point['data']['y'] == point['y'] ){
                return;
            }
            ...
            if (foot=="left"){
                url = Urls.predictedpointleftDetail(point.id)
            }else{
                url = Urls.predictedpointrightDetail(point.id)
            }


            var poiData = {
                "x":point['data']["x"] * point['data']['scale'],
                "y":point['data']["y"] * point['data']['scale'],
            }

            console.log("update",poiData);
            ...
            $.ajax({
                ...
                success:function(response){
                    $.notify("Punkt " + response.id + " gespeichert", "info");
                },
                error:function(response){
                    $.notify("Punkt " + response.id + " konnte nicht gespeichert werden", "error");
                }
            })
        });
        ...
        $.ajax({
            ...
            url: Urls.predictedpointleftList() + "?predictions=" + foamPrintAnalysis,  // Replace <id> with the actual ID of the record you want to update
            ...
            success: function (response) {
                ...
                pointsInDb.forEach(pointInDb => {
                    ...
                    if (!foundPoint) {
                        ...
                        $.ajax({
                            url: Urls.predictedpointleft_detail(pointInDb.id),  // Replace <id> with the actual ID of the record you want to update
                ...
                me.pois.forEach((poi) => {
                    ...
                    if (poi.id == -1) { // this point is not in the db yet, save it
                        $.ajax({
                            url: Urls.predictedpointleft_detail(),  // Replace <id> with the actual ID of the record you want to update
                        ...
                        $.ajax({
                            url: Urls.predictedpointleft_detail(poi.id),  // Replace <id> with the actual ID of the record you want to update
                            ...
                            error: function (error) {
                                ...
                                if (error.status == 404) {
                                    ...
                                    $.ajax({
                                        url: Urls.predictedpointleftList(),  // Replace <id> with the actual ID of the record you want to update
                                        method: 'POST',  // Use 'PUT' if needed
                                        data: JSON.stringify(poiData),
                                        contentType: 'application/json',
                                        success: function (response) {
...
class StateHistory {
    ...
    pushState(state) {
        ...
        var newState = new State(stateCopy.pois);
...
function renderPointEditor(container_id, image_url, points, foot, insole_id, allowEdit = true, initial_zoom = 0.5, window_width=700, window_height=1400, onMeassure = ()=>{}) {
    // Create HTML Stuff
    ...
    points.forEach((point) => {
        currentState.pois.push(new POI(point));
    });
    ...
    var initialState = new State(deepCopy(stateHistory.states[0].pois))
    ...
    canvas.relativePan({ x: panStartX, y: panStartY });
